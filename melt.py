...
class SparseBinaryClassificationTrainer(object):
    def __init__(self, dataset, numClass):
        self.labels = dataset.labels
        self.features = dataset.features
        self.num_features = dataset.num_features
    ...
    def gen_feed_dict(self, trX, trY):
        return {self.Y: trY, self.sp_indices: trX.sp_indices, self.sp_shape: trX.sp_shape,
                self.sp_ids_val: trX.sp_ids_val, self.sp_weights_val: trX.sp_weights_val}
...
def gen_binary_classification_trainer(dataset):
    if type(dataset.features) == Features:
        ...
        return SparseBinaryClassificationTrainer(dataset, 1)


def gen_multi_classification_trainer(dataset, numClass):
...
def prob2Class(p):
    return np.array(p).argmax(axis=1)
